...
void callback(char *topic, byte *payload, unsigned int length)
{
    ...
    else if (strcmp(topic, air_topic) == 0)
    {
        if (strcmp(payloadStr, "ON") == 0)
        {
            digitalWrite(D8, HIGH);
            airState = HIGH;
            client.publish(status_air_topic, "ON");
        }
        else if (strcmp(payloadStr, "OFF") == 0)
        {
            digitalWrite(D8, LOW);
            airState = LOW;
            client.publish(status_air_topic, "OFF");
        }
        statusMsg = "Light: " + String(ledState == HIGH ? "ON" : "OFF") + ", Fan: " + String(fanState == HIGH ? "ON" : "OFF");
        client.publish(status_all_topic, statusMsg.c_str());
    }
...
void reconnect()
{
    while (!client.connected())
    {
        ...
        if (client.connect(clientId.c_str(), mqtt_username, mqtt_password))
        {
            ...
            client.subscribe(status_air_topic);
